...
export class BlockHeader extends Chunk {
	...
	uncompressedLength(value?: number): number {
		let compressed_category = this.uncompressedCategory(value != null ? BlockHeader.getCategory(value) : undefined);
		return BlockHeader.getLength(compressed_category);
	}

	allocatedLength(value?: number): number {
		let category = this.allocatedCategory(value != null ? BlockHeader.getCategory(value) : undefined);
	...
	isCompressed(): boolean {
		return this.isCompressionRequested() && this.uncompressedCategory() > this.allocatedCategory();
	}
...
export class BlockManager {
		...
		if (deleted != null) {
			if (deleted) {
				if (!header.flag1(BlockFlags1.DELETED)) {
					throw new Error(`Expected block to be deleted!`);
				}
			} else {
				if (header.flag1(BlockFlags1.DELETED)) {
					throw new Error(`Expected block to not be deleted!`);
				}
			}
		}
	}
	...
	* [Symbol.iterator](): Iterator<{ bid: number, header: BlockHeader, buffer: Uint8Array }> {
	...
	cloneBlock(idOne: number, compression?: boolean): number {
		...
		let buffer = new Uint8Array(headerOne.allocatedLength());
	...
	readBlock(id: number, data?: Uint8Array, blockOffset?: number): Uint8Array {
		...
		if (blockOffset == null) {
			...
			this.readCompleteBlock(id, blockBuffer);
			data.set(blockBuffer.subarray(0, data.length), 0);
		} else {
			this.readPartialBlock(id, data, activeBlockOffset);
		}
		return data;
	}

	protected readCompleteBlock(id: number, data: Uint8Array): void {
		let header = new BlockHeader();
		this.readBlockHeader(id, header, false);
		let offset = header.offset();
	...
	protected readPartialBlock(id: number, data: Uint8Array, activeBlockOffset: number): void {
		let header = new BlockHeader();
		this.readBlockHeader(id, header, false);
		if (header.isCompressed()) {
			let uncompressedLength = header.uncompressedLength();
			if (data.length !== uncompressedLength) {
				let blockBuffer = new Uint8Array(uncompressedLength);
				this.readCompleteBlock(id, blockBuffer);
				data.set(blockBuffer.subarray(activeBlockOffset, data.length), 0);
			} else {
				this.readCompleteBlock(id, data);
			}
		} else {
			let offset = header.offset();
	...
	resizeBlock(idOne: number, minLength: number): void {
		let headerOne = new BlockHeader();
		this.readBlockHeader(idOne, headerOne, false);
		if (BlockHeader.getCategory(minLength) === headerOne.uncompressedCategory()) {
			return;
		}
	console.log(`Resize block ${idOne} to ${minLength} bytes: ${headerOne}`);
	...
	writeBlock(id: number, data: Uint8Array, blockOffset?: number): Uint8Array {
		...
		if (blockOffset == null) {
			...
			this.writeCompleteBlock(id, blockBuffer);
		} else {
			this.writePartialBlock(id, data, activeBlockOffset);
		}
		return data;
	}

	protected writeCompleteBlock(id: number, data: Uint8Array): void {
		let header = new BlockHeader();
		this.readBlockHeader(id, header, false);
		if (header.isCompressionRequested()) {
			...
			// Temporarily set COMPRESSION_REQUESTED to false to simplify resizing.
			header.flag1(BlockFlags1.COMPRESSION_REQUESTED, false);
			this.writeBlockHeader(id, header);
			this.resizeBlock(id, compressedData.length);
			// Resizing may update the header.
			this.readBlockHeader(id, header, false);
			let blockBuffer = new Uint8Array(header.allocatedLength());
			blockBuffer.set(compressedData, 0);
			this.file.write(blockBuffer, header.offset());
			header.flag1(BlockFlags1.COMPRESSION_REQUESTED, true);
	...
	protected writePartialBlock(id: number, data: Uint8Array, activeBlockOffset: number): void {
		let header = new BlockHeader();
		this.readBlockHeader(id, header, false);
		if (header.isCompressionRequested()) {
			let uncompressedLength = header.uncompressedLength();

...
(0, test_1.test)(`It should support storing block flags.`, async (assert) => {
    ...
    let bh = new vfs.BlockHandler(file);
    let id = bh.createBlock(1);
    bh.setBlockFlag(id, chunks_1.BlockFlags.APPLICATION_0, true);
    assert.true(bh.getBlockFlag(id, chunks_1.BlockFlags.APPLICATION_0) === true);
});
...
(0, test_1.test)(`It should clear block flags when deleting blocks.`, async (assert) => {
    ...
    let bh = new vfs.BlockHandler(file);
    let idOne = bh.createBlock(1);
    bh.setBlockFlag(idOne, chunks_1.BlockFlags.APPLICATION_0, true);
    bh.deleteBlock(idOne);
    let idTwo = bh.createBlock(1);
    assert.true(idOne === idTwo);
    assert.true(bh.getBlockFlag(idTwo, chunks_1.BlockFlags.APPLICATION_0) === false);
});

...
wtf.test(`RLECompressor should compress data.`, async (assert) => {
    let compressed = compressors_1.RLECompressor.compress(Uint8Array.of(0x01, 0x02, 0x03, 0x03, 0x03, 0x03));
...
wtf.test(`RLECompressor should decompress data.`, async (assert) => {
    let decompressed = compressors_1.RLECompressor.decompress(Uint8Array.of(0x01, 0x01, 0x02, 0x82, 0x03), new Uint8Array(6));
...
wtf.test(`RLECompressor should be robust.`, async (assert) => {
    for (let i = 0; i < 100000; i += 1) {
        ...
        let compressed = compressors_1.RLECompressor.compress(array);
        if (compressed == null) {
            continue;
        }
        let decompressed = compressors_1.RLECompressor.decompress(compressed, new Uint8Array(array.length));
